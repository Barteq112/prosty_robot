#include "prosty_robot_description/simple_controller.hpp"
...
SimpleController::SimpleController(const std::string& name)
                                  : Node(name)
{
    declare_parameter("wheel_radius", 0.183);
    declare_parameter("wheel_separation", 0.365);
    ...
    vel_sub_ = create_subscription<geometry_msgs::msg::Twist>("/cmd_vel", 10, std::bind(&SimpleController::velCallback, this, _1));
